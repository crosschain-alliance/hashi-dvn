...
library DVNAdapterMessageCodec {
    ...
    {
        if (_message.length != MESSAGE_SIZE)
            revert DVNAdapter_InvalidMessageSize();

        receiveLib = bytes32(_message[RECEIVE_LIB_OFFSET:PAYLOAD_HASH_OFFSET])
            .toAddress();
        payloadHash = bytes32(
            _message[PAYLOAD_HASH_OFFSET:PACKET_HEADER_OFFSET]
        );
