...
abstract contract DVNAdapterBase is Worker, ILayerZeroDVN {
    ...
    mapping(address sendLib => mapping(uint32 dstEid => bytes32 receiveLib))
        public receiveLibs;
    ...
    function _getAndAssertReceiveLib(
        ...
        if (lib == bytes32(0))
            revert DVNAdapter_MissingRecieveLib(_sendLib, _dstEid);
    }
    ...
    function _assertBalanceAndWithdrawFee(
        address _sendLib,
        uint256 _messageFee
    ) internal {
        ...
        if (balance < _messageFee) {
            ...
            if (balance < _messageFee)
                revert DVNAdapter_InsufficientBalance(balance, _messageFee);
        }
    }
