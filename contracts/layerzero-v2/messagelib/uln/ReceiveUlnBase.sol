...
abstract contract ReceiveUlnBase is UlnBase {
    ...
    function _verify(
        ...
        hashLookup[keccak256(_packetHeader)][_payloadHash][
            msg.sender
    ...
    function _verified(
        ...
        Verification memory verification = hashLookup[_headerHash][
            _payloadHash
        ][_dvn];
        // return true if the dvn has signed enough confirmations
        verified =
            verification.submitted &&
            verification.confirmations >= _requiredConfirmation;
    }
    ...
    function _verifyAndReclaimStorage(
        ...
        if (_config.requiredDVNCount > 0) {
            for (uint8 i = 0; i < _config.requiredDVNCount; ++i) {
                delete hashLookup[_headerHash][_payloadHash][
