...
async function main() {
  ...
  const override = true;
    ...
    if(override){
      ...
      const executor = await hre.ethers.deployContract("MockExecutor",[LZEndpoint,[destEid],msgFee],owner);
  ...
  if(!hre.network.name.includes("localhost"))
  {
   ...
   console.log("HashiDVNFeeLib verified");
    ...
    await hre.run("verify:verify", {
      ...
      constructorArguments: [gnosisEndpoint, await executor.getAddress(), await hashiDVN.getAddress()]
    })
